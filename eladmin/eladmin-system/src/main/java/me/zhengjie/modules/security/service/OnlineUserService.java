...
//        String address = StringUtils.getCityInfo(ip);
        ...
        try {
            onlineUser = new OnlineUser(jwtUser.getId(),jwtUser.getUsername(), jwtUser.getNickName(),jwtUser.getPhone(),jwtUser.getEmail(), dept,jwtUser.getDeptId(),jwtUser.getRoleIds(),jwtUser.isAdmin(), browser , ip, EncryptUtils.desEncrypt(token), new Date());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<OnlineUser> getAll(String filter){
        ...
        for (String key : keys) {
            if(redisUtils.get(key) instanceof OnlineUser) {
    ...
    public void logout(String token) {
        ...
        for (String str:keys){
            Object o = redisUtils.get(str);
            if(o!=null){
                JSONArray array = (JSONArray) o;
                for (int i = 0; i < array.size(); i++) {
                    JSONObject json = array.getJSONObject(i);
                    String ext_token = json.getString("token");
                    String ext_callback = json.getString("callback");
                    Map<String,String> header = new HashMap<>();
                    header.put("Content-Type","application/json; charset=UTF-8");
                    header.put("Authorization",ext_token);
                    HttpRequest post = HttpUtil.createPost(ext_callback);
