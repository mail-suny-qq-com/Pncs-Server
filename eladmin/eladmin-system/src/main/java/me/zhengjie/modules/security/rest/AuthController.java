...
//        final JwtUser jwtUser = (JwtUser) authentication.getPrincipal();
        ...
        Authentication authentication = new UsernamePasswordAuthenticationToken(jwtUser, password, jwtUser.getAuthorities());
        String token = tokenProvider.createToken(authentication);
        String defaultPassword = dictDetailRepository.dictPassword();
        // 保存在线信息
        onlineUserService.save(jwtUser, token, request);
        String pwdState = "safe";
        ...
        authInfo.put("pwdState", pwdState);
        if (singleLogin) {
            //踢掉之前已经登录的token
            onlineUserService.checkLoginOnUser(authUser.getUsername(), token);
        }
