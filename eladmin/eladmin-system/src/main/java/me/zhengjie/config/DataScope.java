...
public class DataScope {
    ...
    public String getDeptIdSql() {
        StringBuilder sb = new StringBuilder(" ( select id from dept where 1=1 ");
        ...
        for (RoleSmallDto role : roleSet) {
            ...
            if (scopeType[1].equals(role.getDataScope())) {
                sb.append(" or id like '"+user.getDept().getId()+"%' ");
            }
            // 存储自定义的数据权限
            if (scopeType[2].equals(role.getDataScope())) {
                Set<Dept> depts = deptService.findByRoleIds(role.getId());
                for (Dept dept : depts) {
                    sb.append(" or id like '"+dept.getId()+"%' ");
                }
            }
        }
        sb.append(" )");
