...
public class LocalStorageController {
    ...
    @ApiOperation("查询制度文件")
    @GetMapping(value = "/queryForPortal")
    public ResponseEntity getLocalStoragesByVisible(LocalStorageQueryCriteria criteria,Pageable pageable){
        criteria.setVisible("1");
        return new ResponseEntity<>(localStorageService.queryAll(criteria,pageable),HttpStatus.OK);
    }
    ...
    public ResponseEntity<Object> create(@RequestParam String name,@RequestParam String portal_visible, @RequestParam("file") MultipartFile file){
        return new ResponseEntity<>(localStorageService.create(name, portal_visible,file),HttpStatus.CREATED);
    }
