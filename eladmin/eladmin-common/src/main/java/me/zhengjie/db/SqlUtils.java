...
public class SqlUtils {
	...
	private static DruidDataSource getDataSource(String jdbcUrl, String userName, String password) {
		...
		druidDataSource.setDriverClassName(dataTypeEnum.getDriver());
		if(StrUtil.isNotEmpty(dataTypeEnum.getValidationQuery())){
			druidDataSource.setValidationQuery(dataTypeEnum.getValidationQuery());
		}
	...
	public static DataSource getDataSource(String dbId) {
		if(StrUtil.isEmpty(dbId)){
			throw new RuntimeException("dbId为空!");
		}
		dbId = dbId.toLowerCase();
		if (!map.containsKey(dbId) || null == map.get(dbId)) {
			JSONObject object = configMap.get(dbId);
			if(object==null){
				throw new RuntimeException("dbId未定义,dbId = " + dbId);
			}
			String jdbcUrl = object.getString("jdbcUrl");
			String userName = object.getString("userName");
			String password = object.getString("password");

			DruidDataSource druidDataSource = getDataSource(jdbcUrl,userName,password);
			map.put(dbId, druidDataSource);
		}
		return map.get(dbId);
	}

	public static void removeDataSource(String dbId){
		configMap.remove(dbId);
		map.remove(dbId);
	}
	...
	public static Connection getConnection(String dbId) {
		...
		DataSource dataSource = map.get(dbId);
		...
		try {
			...
			throw new RuntimeException("create connection error, dbId: " + dbId);
		}
		return connection;
	}
	...
	public static String executeFile(Connection connection, File sqlFile) {
		try {
