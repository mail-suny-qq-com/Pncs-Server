...
public class DatabaseServiceImpl implements DatabaseService {
    ...
    public DatabaseDto create(Database resources) {
        ...
        object.put("jdbcUrl",resources.getJdbcUrl());
        object.put("userName",resources.getUserName());
        object.put("password",resources.getPwd());
        me.zhengjie.db.SqlUtils.getConfigMap().put(resources.getName().toLowerCase(),object);
    ...
    public void delete(Set<String> ids) {
        for (String id : ids) {
            //删除数据源配置
            Database db = databaseRepository.findById(id).get();
            me.zhengjie.db.SqlUtils.removeDataSource(db.getName().toLowerCase());
