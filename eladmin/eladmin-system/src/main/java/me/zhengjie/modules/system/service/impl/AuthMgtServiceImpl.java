...
public class AuthMgtServiceImpl implements AuthMgtService {
    ...
    public List<String> getAdminMenus(String type, String id) {
        String sql = " select menu_id from object_menus where object_type=:type and object_id=:id ";
    ...
    public void saveAdminMenus(ObjectMenus object) {
        ...
        entityManager.createNativeQuery("delete from object_menus where object_type=:type and object_id=:id").setParameter("type",type).setParameter("id",id).executeUpdate();
        for (String menuId:object.getMenuIds()){
            entityManager.createNativeQuery("insert into object_menus values(?,?,?)")
