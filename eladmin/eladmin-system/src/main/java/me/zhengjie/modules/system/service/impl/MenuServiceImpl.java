...
//    }
    ...
    public Set<Menu> getDeleteMenus(List<Menu> menuList, Set<Menu> menuSet) {
        ...
        for (Menu menu1 : menuList) {
            menuSet.add(menu1);
            List<Menu> menus = menuRepository.findByPidOrderBySortAsc(menu1.getId());
    ...
    public Object getMenuTree(List<Menu> menus) {
        ...
        menus.forEach(menu -> {
                    if (menu!=null){
                        List<Menu> menuList = menuRepository.findByPidOrderBySortAsc(menu.getId());
    ...
    public List<MenuVo> buildMenus(List<MenuDto> menuDtos) {
        ...
        menuDtos = menuDtos.stream().sorted(Comparator.comparing(MenuDto::getSort))
        ...
        menuDtos.forEach(menuDTO -> {
            if (menuDTO!=null){
                ...
                // 一级目录需要加斜杠，不然会报警告
                menuVo.setPath(menuDTO.getPid().equals("0") ? "/" + menuDTO.getPath() :menuDTO.getPath());
                ...
                if(menuDtoList !=null && menuDtoList.size()!=0){
                    ...
                    // 处理是一级菜单并且没有子菜单的情况
                } else if(menuDTO.getPid().equals("0")){
    ...
    public List<Menu> findObjectMenus(String type, String id) {
        String sql = "select * from sys_menu where type!='2' and id in (select menu_id from sys_object_menu where object_type=:type and object_id=:id) ";
