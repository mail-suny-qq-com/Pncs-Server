...
public class AuthMgtServiceImpl implements AuthMgtService {
    ...
    public void saveAdminMenus(ObjectMenus object) {
        ...
        for (String menuId : object.getMenuIds()) {
            entityManager.createNativeQuery("insert into sys_object_menu(object_type,object_id,menu_id,id) values(?,?,?,?)")
                    .setParameter(1, type).setParameter(2, id).setParameter(3, menuId).setParameter(4, UUID.randomUUID().toString().replaceAll("-", "")).executeUpdate();
        }
    }
}
