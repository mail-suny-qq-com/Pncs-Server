...
//        List<MenuDto> menuDtoList = menuService.findByRoles(roleService.findByUsersId(user.getId()));
        ...
        //查询机构菜单
        List<Menu> deptMenus = menuService.findObjectMenus("dept",user.getDept().getId());
        for (Menu menu:deptMenus){
            MenuDto dto = menuMapper.toDto(menu);
            if(!menuDtoList.contains(dto)){
                menuDtoList.add(dto);
            }
        }
        //查询角色菜单
        List<RoleSmallDto> roleList = roleService.findByUsersId(user.getId());
        for (RoleSmallDto roleDto:roleList){
            List<Menu> roleMenus = menuService.findObjectMenus("role",roleDto.getId());
        ...
        //查询用户菜单
        List<Menu> userMenus = menuService.findObjectMenus("user",user.getId());
    ...
    @PreAuthorize("@el.check('menu:list','roles:list','auth:list')")
