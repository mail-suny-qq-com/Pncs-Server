...
public class UserController {
    ...
    public UserController(DictDetailRepository dictDetailRepository, PasswordEncoder passwordEncoder, UserService userService, DataScope dataScope, DeptService deptService, RoleService roleService, VerificationCodeService verificationCodeService) {
    ...
    @ApiOperation("查询用户")
    @GetMapping
    @PreAuthorize("@el.check('user:list','auth:list')")
    public ResponseEntity<Object> getUsers(UserQueryCriteria criteria, Pageable pageable){
        /*Set<String> deptSet = new HashSet<>();
        Set<String> result = new HashSet<>();
        //顶级机构不取
        if (!ObjectUtils.isEmpty(criteria.getDeptId()) && !"1".equals(criteria.getDeptId())) {
...
//        Set<String> deptIds = dataScope.getDeptIds();
        ...
        if ("1".equals(criteria.getDeptId())) {
    ...
    @PreAuthorize("@el.check('user:list','auth:list')")
    public ResponseEntity<Object> getUserList(UserQueryCriteria criteria, Pageable pageable){
        return new ResponseEntity<>(userService.queryUserList(criteria,pageable),HttpStatus.OK);
    }
...
//        checkLevel(resources);
        // 默认密码 123456
        String defaultPassword = dictDetailRepository.dictPassword();
        resources.setPassword(passwordEncoder.encode(defaultPassword));
...
//        checkLevel(resources);
    ...
    public ResponseEntity<Object> resetPwd(@RequestBody Set<String> ids){
        for (String id : ids) {
            UserDto user = userService.findById(id);
            String defaultPassword = dictDetailRepository.dictPassword();
            userService.updatePass(user.getUsername(),passwordEncoder.encode(defaultPassword));
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
}
