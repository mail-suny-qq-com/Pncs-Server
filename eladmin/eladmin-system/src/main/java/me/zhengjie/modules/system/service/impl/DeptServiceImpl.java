...
//    @Cacheable
    public Object buildTree(List<DeptDto> deptDtos) {
        ...
        List<String> deptNames = deptDtos.stream().map(DeptDto::getId).collect(Collectors.toList());
        boolean isChild;
...
//            else if(!deptNames.contains(deptRepository.findNameById(deptDTO.getPid()))) {
            else if(!deptNames.contains(deptDTO.getPid())) {
                depts.add(deptDTO);
            }
        }
        List<DeptDto> toRemoveList = new ArrayList<>();
        for(DeptDto deptDto:depts){
            for(DeptDto dto:depts){
                if(!CollectionUtils.isEmpty(deptDto.getChildren()) && deptDto.getChildren().contains(dto)){
                    toRemoveList.add(dto);
                }
            }
        }
        depts.removeAll(toRemoveList);
    ...
    public DeptDto create(Dept resources) {
        String maxId = deptRepository.findMaxIdByPid(resources.getPid());
        String id = resources.getPid() + "01";
        if(StrUtil.isNotEmpty(maxId)){
